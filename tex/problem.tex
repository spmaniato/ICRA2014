...
Using the terminology in Section \ref{preliminaries}, we can express the notion of a \emph{closed} world. Specifically, in our framework, the closed world assumption implies that $AP$, the set of atomic propositions from which LTL specifications are constructed, is fixed. That is, if the robot encounters a new element in its world (such as a new object), which was not modeled as a proposition $\pi \in AP$, it will essentially ignore it, even if it is relevant to the mission.

Three main reasons motivate the need for open world mission specification and planning. First of all, in real-world applications, such as autonomous search and rescue scenarios, the mission is often specified before the robot has obtained full knowledge of the world. Second, the robot may have to incorporate new function and components to its strategy, i.e., add new propositions to $AP$. Finally, the mission's set of atomic propositions $AP$ may be too large for the user to enumerate a priori.  
...
In addition, let $D$ be a function that reads the sensors that environment propositions $\mathcal{D}$ abstract, and returns a new proposition if they detected an unmodeled element of the world:
...
\begin{myDefinition}\label{Def:openworld}	
	...
	\begin{equation}\label{Eq:updateAP}
		AP_{k+1} = AP_k \bigcup_{m=1}^{M}D(m, k),
	\end{equation}
...
Notice the assumption regarding $\W$ that Eq. \eqref{Eq:updateAP} makes:

\begin{myAssumption}\label{Ass:noRemove}
	The open world model $\W$ only models the addition of new propositions as a result of new elements of the open world. That is, $AP_{k} \subset AP_{k+1}$. In other words, no elements are removed from the open world.
\end{myAssumption}

However, Assumption \ref{Ass:noRemove} is not really limiting, since a proposition can be in $AP$ without being part of the discrete strategy. Furthermore, if necessary, the value of a proposition can be restricted by adding safety requirements to $\varphi_t^{e,s}$. The disadvantage of not being able to remove propositions no longer pertinent to the mission is that the synthesis algorithm is searching for a winning strategy over a larger graph, which increases computation time.
% Feedback on this Assumption + comments.
% graph, game structure, something else?

In order to obtain a mission specification and a robot plan that react to unmodeled elements of the world, we need to address two main problems. First of all, we should allow specifications that include formulas outside LTL, in order to enable the mission to adapt to $\W$. Then, we need to define the specific mechanics that will allow new propositions $\pi = D(m,k)$ to be incorporated in the mission's LTL formulas.

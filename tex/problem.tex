...
In addition, let $D$ be a function that reads the sensors that the environment propositions $\mathcal{D}$ abstract, and returns a new proposition if the sensors detected an unmodeled element of the world:
...
\begin{myAssumption}\label{Ass:noRemove}
	$\W$ only models the {\em addition} of propositions as a result of new elements of the open world. That is, $AP_{k} \subset AP_{k+1}$. In other words, propositions cannot be removed. Hence, $\W$ is dubbed an \emph{expanding} open world model.
\end{myAssumption}

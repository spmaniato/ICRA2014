...
\begin{equation}\label{Eq:A}
	A(m, k) = 
	\begin{cases}
		\left\{ \pi_{mk} \right\}, & \text{if } d_m \\ % Refers to valuation {0,1}
		\left\{ \right\}, &  \text{if } \neg d_m
	\end{cases}
\end{equation}¥
where $d_m \in \mathcal{D}$ and $\pi_{mk} \not \in AP_k$. The new proposition $\pi_{mk}$ can be either an environment, action, or region proposition.
...
In order to obtain a mission specification and a robot plan that can react to unmodeled elements of the world, we need to address two main problems. First of all, we should allow mission specifications

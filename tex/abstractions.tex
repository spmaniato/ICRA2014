...
\begin{align}
	&G \subseteq AP_t \\
	&\pi \in S_t \forall \pi \in G | \pi \in A_t \forall \pi \in G | \pi \in R_t \forall \pi \in G
\end{align}
\end{subequations}
Where $S_t \subseteq AP_t$ is the set of sensor propositions at time $t$, $A_t \subseteq AP_t$ is the set of action propositions at time $t$, and $R_t \subseteq AP_t$ is the set of region propositions at time $t$. 
...
For quantification to be meaningful in a robot's behavior the parser $\mathcal{P}$ must be able to translate the semantics of these first-order logic operations into LTL formulas in $\varphi$.
